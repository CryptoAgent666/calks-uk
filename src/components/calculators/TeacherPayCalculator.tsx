  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div><label className="block text-sm font-medium mb-2">Pay Scale</label><select value={scale} onChange={(e) => { setScale(e.target.value as Scale); setPoint(0) }} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"><option value="main">Main Pay Scale (M1-M6)</option><option value="upper">Upper Pay Scale (UPS1-3)</option><option value="leadership">Leadership</option></select></div>
        {scale !== 'leadership' ? (
          <div><label className="block text-sm font-medium mb-2">Pay Point</label><select value={point} onChange={(e) => setPoint(parseInt(e.target.value))} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring">{(scale === 'main' ? MAIN_SCALE : UPPER_SCALE).map((_, i) => <option key={i} value={i}>{scale === 'main' ? `M${i+1}` : `UPS${i+1}`}</option>)}</select></div>
        ) : (
          <><div><label className="block text-sm font-medium mb-2">Leadership Group</label><select value={leadership} onChange={(e) => setLeadership(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring">{Object.keys(LEADERSHIP).map(k => <option key={k} value={k}>{k}</option>)}</select></div>
          <div><label className="block text-sm font-medium mb-2">Point in Range (%)</label><input type="range" min="0" max="100" value={point} onChange={(e) => setPoint(parseInt(e.target.value))} className="w-full mt-3" /></div></>
        )}
        <div><label className="block text-sm font-medium mb-2">London</label><select value={london} onChange={(e) => setLondon(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"><option value="none">Rest of England</option><option value="inner">Inner London (+£5K)</option><option value="outer">Outer London (+£2K)</option><option value="fringe">London Fringe (+£1K)</option></select></div>
      </div>
