  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div><label className="block text-sm font-medium mb-2">Roof Length (m)</label><input type="number" min="0" step="0.1" value={length} onChange={(e) => setLength(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"  aria-label="Roof Length (m)" /></div>
        <div><label className="block text-sm font-medium mb-2">Building Width (m)</label><input type="number" min="0" step="0.1" value={width} onChange={(e) => setWidth(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"  aria-label="Building Width (m)" /></div>
        <div><label className="block text-sm font-medium mb-2">Roof Pitch (degrees)</label><input type="number" min="15" max="60" value={pitch} onChange={(e) => setPitch(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"  aria-label="Roof Pitch (degrees)" /></div>
        <div><label className="block text-sm font-medium mb-2">Tile Type</label><select value={tile} onChange={(e) => setTile(e.target.value as TileType)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring" aria-label="Tile Type"><option value="concrete">Concrete (~10/m²)</option><option value="clay">Clay (~15/m²)</option><option value="slate">Slate (~20/m²)</option></select></div>
        <div><label className="block text-sm font-medium mb-2">Wastage (%)</label><input type="number" min="0" max="20" value={waste} onChange={(e) => setWaste(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"  aria-label="Wastage (%)" /></div>
      </div>
