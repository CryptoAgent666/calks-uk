...
function calculate(carValue: number, shippingCost: number, co2: number, fuelType: string, isEU: boolean) {
  ...
  // Registration: first year VED
  let ved = 0
  if (fuelType === 'electric') ved = 10
  else if (co2 <= 50) ved = 10
  else if (co2 <= 75) ved = 30
  else if (co2 <= 90) ved = 135
  else if (co2 <= 100) ved = 175
  else if (co2 <= 130) ved = 220
  else if (co2 <= 150) ved = 270
  else if (co2 <= 170) ved = 680
  else ved = 1_095
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        <div><label className="block text-sm font-medium mb-2">CO2 (g/km)</label><input type="number" min="0" max="300" value={co2} onChange={(e) => setCo2(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring" /></div>
