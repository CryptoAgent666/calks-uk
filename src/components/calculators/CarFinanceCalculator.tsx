  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        <div><label className="block text-sm font-medium mb-2">APR (%)</label><input type="number" min="0" max="30" step="0.1" value={apr} onChange={(e) => setApr(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"  aria-label="APR (%)" /></div>
        ...
        {type === 'pcp' && <div><label className="block text-sm font-medium mb-2">Balloon / GFV (%)</label><input type="number" min="10" max="60" value={balloon} onChange={(e) => setBalloon(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"  aria-label="Balloon / GFV (%)" /></div>}
      </div>
