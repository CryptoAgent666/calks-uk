  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div><label className="block text-sm font-medium mb-2">Boiler Type</label><select value={type} onChange={(e) => setType(e.target.value as BoilerType)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"><option value="combi">Combi</option><option value="system">System</option><option value="regular">Regular</option></select></div>
        <div><label className="block text-sm font-medium mb-2">Current Boiler</label><select value={oldEff} onChange={(e) => setOldEff(e.target.value as OldEfficiency)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"><option value="old">15+ years old (~65%)</option><option value="medium">10-15 years (~78%)</option><option value="good">5-10 years (~85%)</option></select></div>
