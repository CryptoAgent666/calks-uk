  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div><label className="block text-sm font-medium mb-2">Hectares</label><input type="number" min="1" max="5000" value={ha} onChange={(e) => setHa(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring" /></div>
        <div><label className="block text-sm font-medium mb-2">Land Type</label><select value={type} onChange={(e) => setType(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"><option value="arable">Arable (~£250/ha)</option><option value="grassland">Grassland (~£150/ha)</option><option value="mixed">Mixed (~£200/ha)</option><option value="upland">Upland (~£80/ha)</option></select></div>
