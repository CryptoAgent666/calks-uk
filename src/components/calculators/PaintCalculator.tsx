  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        <div><label className="block text-sm font-medium mb-2">Number of Coats</label><input type="number" min="1" max="5" value={coats} onChange={(e) => setCoats(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"  aria-label="Number of Coats" /></div>
        <div><label className="block text-sm font-medium mb-2">Doors/Windows (m²)</label><input type="number" min="0" step="0.1" value={doors} onChange={(e) => setDoors(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"  aria-label="Doors/Windows (m²)" /></div>
        <div><label className="block text-sm font-medium mb-2">Coverage (m²/litre)</label><input type="number" min="1" max="20" value={coverage} onChange={(e) => setCoverage(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 font-medium focus:outline-none focus:ring-2 focus:ring-ring"  aria-label="Coverage (m²/litre)" /></div>
      </div>
