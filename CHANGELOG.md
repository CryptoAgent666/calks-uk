...
First iOS build of Calks UK (`uk.calks.app`), mirroring the existing Play Store app and the approved Calk KZ / Calk KG pattern. Works **fully offline** (entire site bundled in the `.app`) but **pulls new web bundles** (formulas, new calculators, content fixes) from Cloudflare when online — no App Store resubmission needed for web changes.
- `npx cap add ios` → `ios/` project (Capacitor 8 = **Swift Package Manager**, no CocoaPods). `cap sync ios` registers 4 native plugins: app, splash-screen, status-bar, **@capgo/capacitor-updater**.
- **OTA engine: `@capgo/capacitor-updater` 8.47.5**, self-hosted on Cloudflare (no Capgo cloud account). `capacitor.config.ts` → `CapacitorUpdater` in **manual mode** (`autoUpdate:false`, `appReadyTimeout:15000`, `resetWhenUpdate:true`).
- **`src/scripts/ota-update.ts`** (new) — on launch, in the native app only, when online: `notifyAppReady()` → fetch `https://calks.uk/app/latest.json` → if its `version` > the running bundle's baked `PUBLIC_BUILD_ID` (and not already downloaded) → `download()` the zip → `next()` to activate on the **next launch/resume** (never mid-session). Guards: native-only, once-per-session, offline-safe, `localStorage` de-dupe.
  - The Capgo plugin is **dynamically imported** *after* the native-app guard, so it code-splits into a lazy chunk — **web visitors never download it** (verified: web pages load only a 1.2 KB guard; the 17 KB Capgo chunk is referenced nowhere in HTML). Wired into `Layout.astro` as a bundled (non-inline) `<script>`.
- **`scripts/deploy.mjs`** (new) + `npm run deploy` — one command: build with a stamped `PUBLIC_BUILD_ID` (YYYYMMDDHHMM) → zip the clean `dist` as the OTA bundle → stage `bundle-<v>.zip` + `latest.json` under `dist/app/` (added **after** zipping, so the bundle never contains itself) → `wrangler pages deploy dist`. Site + OTA manifest go live together; installed apps pick up the new bundle on next launch.
- **`package.json`**: added `build:ios` (`PUBLIC_BUILD_ID=$(date …) astro build && cap sync ios`), `deploy`; `build:android` now also stamps the build id (same OTA mechanism works for Android).
- **Branding**: brand lockup = white calculator glyph + **"Calks.UK" wordmark** on navy `#1e3a5f`. iOS app icon (1024×1024, **RGB no-alpha** — App Store compliant) + navy launch/splash (icon lockup centred; storyboard bg set to navy, no white flash). Android icons regenerated to match: adaptive `ic_launcher_foreground.png` (wordmark kept inside the Ø66dp safe zone) across 5 densities + legacy square/round (48–192). Sources: `resources/icon.svg`, `resources/android-foreground.svg`, `resources/splash-logo.svg`.
- **Validation**: `xcodebuild … -sdk iphonesimulator` → **BUILD SUCCEEDED**, 0 errors; Capgo + Alamofire/ZIPFoundation/BigInt compiled and linked; `App.app/public/index.html` confirms the offline bundle is embedded.
- **Submission prep**: Capacitor suite aligned to **8.4.0** (core/cli/android/ios — clears the core/android mismatch warning); `ITSAppUsesNonExemptEncryption=false` added to `Info.plist` (standard HTTPS → skips the export-compliance prompt each upload); App Store listing drafted in `app-store-assets/listing.md` + 5 screenshots (1320×2868, 6.9") in `app-store-assets/screenshots-ios/`; **`PrivacyInfo.xcprivacy`** added to the app target (no tracking / no data collected + UserDefaults `CA92.1` & file-timestamp `C617.1` required-reason APIs used by Capgo → avoids ITMS-91053), wired into the Xcode Resources phase and verified bundled in the `.app`; **iOS Simulator smoke-test passed** — branded navy splash + homepage render fully offline in the native webview. Android re-synced with Capgo + the new "Calks.UK" icons (ready for a Play Store update).
- **Handoff (needs the Apple dev account — same as KZ/KG)**: open `ios/App/App.xcodeproj` in Xcode → set Team + signing on the `App` target → archive → upload to App Store Connect (listing text ready in `app-store-assets/listing.md`; capture iOS screenshots from the Simulator). To ship a web update afterward: just `npm run deploy`.
